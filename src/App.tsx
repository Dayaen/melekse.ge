...
function WordTip({ token }: WordTipProps) {
  ...
  return (
    ...
    >
      <span className="border-b-2 border-dotted border-red-600/50 hover:border-[#9E1218] hover:text-[#9E1218] transition-all duration-200 font-medium pb-0.5 px-0.5 rounded-sm hover:bg-red-600/5">
        {token.text}
      </span>
      {isOpen && (
        <span
  ...
  return (
    <li
      ...
      className={`relative py-1 px-2.5 my-0.5 rounded-md border cursor-pointer overflow-hidden transition-all duration-300 font-semibold select-none group
        ${active 
          ? "bg-[#9E1218]/5 text-[#9E1218] border-[#9E1218]/25 shadow-xs font-bold" 
          : "bg-transparent text-stone-700 hover:text-stone-950 border-transparent hover:bg-stone-800/10"
        }`}
    >
      <div className="relative z-10 flex flex-col items-start gap-0.5">
        ...
        <span className="text-[9px] uppercase tracking-wider text-stone-500 font-mono group-hover:text-[#9E1218]/95 transition-colors">
          {chapter.rangeText}
        </span>
      </div>
      ...
      {isScratching && (
        <div className="absolute inset-0 bg-red-600/5 border border-red-600/20 backdrop-blur-[0.5px] rounded-lg pointer-events-none animate-pulse" />
      )}
    </li>
  );
}
...
export default function App() {
  ...
  const currentIndex = useMemo(() => {
    return CHAPTERS.findIndex((ch) => ch.id === activeChapterId);
  }, [activeChapterId]);
  ...
  const handleChapterSelect = (chapterId: string) => {
    ...
    // Smooth scroll to top of stanzas container
  ...
  const themeClasses = {
    parchment: {
      bg: "bg-[radial-gradient(ellipse_at_top,#FCFAF7_0%,#F3EFE9_60%,#E6E0D4_100%)]",
      overlay: "opacity-[0.03] mix-blend-multiply",
      card: "bg-white/95 border-stone-200/80 shadow-[0_4px_24px_-4px_rgba(45,27,16,0.06)]",
      textMain: "text-stone-900",
      textMuted: "text-stone-600/90",
      accent: "text-[#9E1218]",
      sidebar: "bg-[#FAF7F1]/95 border-r border-stone-200/60",
      header: "bg-[#FCFAF7]/95 border-b border-stone-200/60",
      searchBg: "bg-stone-50 border-stone-200 text-stone-900 focus:ring-[#9E1218]",
      btnActive: "bg-[#9E1218] text-stone-50 shadow-md",
      btnInactive: "bg-[#F3EFE9] text-stone-800 border-stone-200/50 hover:bg-[#EBE7DC]"
    },
    dark: {
    ...
    <div id="manuscript_root" className={`relative flex flex-col h-screen w-full select-none font-sans-geo ${themeClasses.bg} text-stone-900 transition-colors duration-500 overflow-hidden`}>
      ...
      <header id="manuscript_header" className={`relative z-30 flex items-center justify-between px-4 sm:px-6 py-4 backdrop-blur-md ${themeClasses.header} transition-all border-b border-black/5`}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <h1 className="font-serif-geo text-2xl sm:text-4.5xl font-extrabold tracking-widest text-[#9E1218] drop-shadow-xs">
            ვეფხისტყაოსანი
          </h1>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2.5 mt-1 font-sans-geo text-xs sm:text-[13px]">
            ...
            <AnimatePresence mode="wait">
              <motion.span
                ...
                className="text-[#9E1218] font-serif-geo italic font-semibold px-2.5 py-0.5 bg-[#9E1218]/5 rounded-md border border-[#9E1218]/15 shadow-2xs text-center"
              >
      ...
      <div id="manuscript_workspace" className="flex flex-1 relative overflow-hidden z-10">
        ...
        <main id="manuscript_main_view" className="flex-1 flex flex-col overflow-hidden relative">
          ...
          >
            ...
            <span className="lg:hidden text-[10px] font-sans-geo font-bold uppercase tracking-wider text-[#9E1218] bg-[#9E1218]/5 px-2 py-0.5 rounded-md border border-[#9E1218]/10">
          ...
          <AnimatePresence>
            {isSidebarOpen && (
              <motion.div
                ...
                className="lg:hidden w-full border-b border-stone-200/80 overflow-hidden bg-[#FCFAF7] text-stone-900 shadow-xl z-20 max-h-[50vh] overflow-y-auto"
              >
                <div className="p-3 flex flex-col gap-1 pr-3 custom-scrollbar">
                  <div className="text-[10px] uppercase font-mono tracking-widest text-[#9E1218] font-extrabold px-1 py-1 mb-1 border-b border-stone-200/50">
                    აირჩიეთ თავი:
                  </div>
                  {CHAPTERS.map((chapter) => (
                    <button
                      ...
                      className={`w-full text-left py-2 px-3 rounded-lg border transition-all duration-200 flex flex-col items-start gap-1 
                        ${activeChapterId === chapter.id 
                          ? "bg-[#9E1218]/10 text-[#9E1218] border-[#9E1218]/25 font-bold" 
          ...
          <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-2 sm:p-3.5 custom-scrollbar space-y-4">
            <div className="max-w-full mx-auto w-full px-1 lg:px-2.5 space-y-5">
                    ...
                    <div className={`flex-1 space-y-3 relative ${showParaphraseMap[stanza.id] ? "hidden lg:block" : "block"}`}>
                      <div className="flex items-center justify-between select-none">
                        ...
                        <button
                          ...
                          className="lg:hidden text-[11px] px-3 py-1 rounded-lg font-sans-geo font-bold shadow-xs transition-all duration-300 bg-[#9E1218] text-stone-50 hover:bg-[#9E1218]/90 flex items-center gap-1 border border-[#9E1218]/20"
                        >
                          შინაარსი 📖
                        </button>
                      </div>
              ...
              <div className="pt-8 pb-12 border-t border-stone-200/60 mt-10 flex flex-col sm:flex-row items-center justify-between gap-4 select-none">
                {prevChapter ? (
                  <button
                    onClick={() => handleChapterSelect(prevChapter.id)}
                    className="w-full sm:w-auto px-5 py-3 rounded-xl border border-stone-200/85 hover:border-[#9E1218]/30 bg-white/70 hover:bg-[#9E1218]/4 hover:shadow-xs transition-all duration-300 flex items-center justify-center sm:justify-start gap-4 group cursor-pointer"
                  >
                    <ArrowLeft size={16} className="text-[#9E1218] transition-transform duration-300 group-hover:-translate-x-1" />
                    <div className="flex flex-col items-start text-left">
                      <span className="text-[9px] uppercase tracking-wider text-stone-550 font-mono">წინა თავი</span>
                      <span className="font-serif-geo text-[13px] font-semibold text-stone-800 group-hover:text-[#9E1218] transition-colors">{prevChapter.title}</span>
                    </div>
                  </button>
                ) : (
                ...
                {nextChapter ? (
                  <button
                    ...
                    className="w-full sm:w-auto px-5 py-3 rounded-xl border border-stone-200/85 hover:border-[#9E1218]/30 bg-white/70 hover:bg-[#9E1218]/4 hover:shadow-xs transition-all duration-300 flex items-center justify-center sm:justify-end gap-4 group cursor-pointer ml-auto"
                  >
                    <div className="flex flex-col items-end text-right">
                      <span className="text-[9px] uppercase tracking-wider text-stone-550 font-mono">შემდეგი თავი</span>
                      <span className="font-serif-geo text-[13px] font-semibold text-stone-800 group-hover:text-[#9E1218] transition-colors">{nextChapter.title}</span>
                    </div>
                    <ArrowRight size={16} className="text-[#9E1218] transition-transform duration-300 group-hover:translate-x-1" />
                  </button>
                ) : (
                  <div className="hidden sm:block" />
                )}
